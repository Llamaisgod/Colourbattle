...
const W = 1280; // Was 1920
const H = 720;  // Was 1080
canvas.width = W;
canvas.height = H;

// Handle window resizing implies we just keep the canvas resolution fixed
window.addEventListener('resize', () => {});

// --- HELPER: Coordinate Scaling ---
// This fixes the "Zoom Glitch" and "Card Selection" by mapping screen pixels to canvas pixels.
...
// FIX: Updated mouse listeners to use scaling
...
// FIX: Lava Not Showing
// Moved 'lavaYFrac' out of the 'platforms' array and into the root object for Maps 0 and 1.
const MapTemplates = [
  { id:0, name:'Training Grounds', lavaYFrac: 0.9, platforms:[ {cx:0.25,cy:0.72,wFrac:0.24,h:28},{cx:0.75,cy:0.72,wFrac:0.24,h:28},{cx:0.5,cy:0.48,wFrac:0.26,h:28}], spawnA:{cx:0.2,cy:0.5}, spawnB:{cx:0.8,cy:0.5} },
  { id:1, name:'Sky Islands', lavaYFrac: 0.9, platforms:[ {cx:0.18,cy:0.66,wFrac:0.2,h:20},{cx:0.5,cy:0.5,wFrac:0.22,h:20},{cx:0.82,cy:0.66,wFrac:0.2,h:20}], spawnA:{cx:0.18,cy:0.46}, spawnB:{cx:0.82,cy:0.46} },
  { id:2, name:'Lava Pit', lavaYFrac: 0.9, platforms:[ {cx:0.25,cy:0.70,wFrac:0.26,h:26},{cx:0.75,cy:0.70,wFrac:0.26,h:26}], spawnA:{cx:0.2,cy:0.48}, spawnB:{cx:0.8,cy:0.48} }
];
...
class Player {
  constructor(id,x,y,color){
    ...
    this.maxMag=4; this.mag=4; this.reload=0; this.reloadTime = 1200; this.attackSpeed=1; this.moveSpeedMultiplier=1; this.bulletSpeedMultiplier=1; this.lifesteal=0; this.scavenger=0; this.tasteOfBlood_ms=0;
...
class Bullet {
  ...
  update(){
    ...
    if(!this.sneaky && !this.remote){
      this.vy += (currentMap.gravity || 0.45) * 0.05; 
    }
    this.x += this.vx;
    this.y += this.vy;
    ...
    if (scores.p1 >= TARGET_SCORE || scores.p2 >= TARGET_SCORE) {
        ...
        setTimeout(() => {
            ...
            for (let i = 0; i < Players.length; i++) {
                ...
                const fresh = new Player(old.id, spawn.x, spawn.y, baseColor);
...
function tryFire(player){
  ...
  if(player.multishotEnabled && player.bulletsPerShot > 1){ const spread = player.spread || 0.28; const n = player.bulletsPerShot; for(let i=0;i<n;i++){ const offset = ((i - (n-1)/2) / Math.max(1,(n-1))) * spread; fireSingleBullet(player, offset); } } else if(player.burstEnabled && player.burstCount > 1){ for(let i=0;i<player.burstCount;i++){ Scheduler.schedule(i * player.burstDelay, ()=> fireSingleBullet(player) ); } } else {
...
// FIX: Updated click listener to use scaling for accurate card selection
canvas.addEventListener('mousedown', e=>{
    mouse.down=true;
    if(showSplash) startSequence();
    if(pickState && pickState.active){
        const pos = getMousePos(e); // Use the fixed coordinate scaler
        const mx = pos.x;
        const my = pos.y;
        
        const boxW=Math.min(W*0.92,960), boxH=Math.min(H*0.78,360);
        const bx=(W-boxW)/2, by=(H-boxH)/2;
        const cardW=(boxW-80)/5, cardH=boxH-100;
        
        for(let i=0;i<pickState.options.length;i++){
            const cx=bx + 40 + i*cardW, cy=by + 60;
            if(mx>=cx && mx<=cx+cardW && my>=cy && my<=cy+cardH){
                const player = pickState.currentPicker==='p1' ? Players[0] : Players[1];
                player.applyCard(pickState.options[i]);
                pickState.active=false;
                pickState.chosen=pickState.options[i];
                spawnParticles(mx,my,20,'#ffd27a');
                setTimeout(()=> showNextPick(),280);
                break;
            }
        }
    }
...
function tick(now){ const dt = Math.min(40, now - lastTime); lastTime = now; frames++; fpsTimer += dt; if(fpsTimer >= 1000){ fps = frames; frames = 0; fpsTimer = 0; } Scheduler.update(); const p1input = { left: keys['a'], right: keys['d'], jump: keys['w'], shoot: keys['t'], block: keys['Shift'] || keys['ShiftLeft'] }; const p2input = { left: keys['ArrowLeft'], right: keys['ArrowRight'], jump: keys['ArrowUp'], shoot: keys['l'], block: keys['Control'] || keys['ControlLeft'] }; if(pickState.active){ Players.forEach((p,i)=> p.update(null, dt, i===0?Players[1]:Players[0])); } else if(inRound){ Players.forEach((p,i)=> p.update(i===0? p1input : p2input, dt, i===0?Players[1]:Players[0]));
    if(keyPress['t']){ const p = Players[0]; tryFire(p); keyPress['t'] = false; }
    if(keyPress['l']){ const p = Players[1]; tryFire(p); keyPress['l'] = false; }
    for(let i=Bullets.length-1;i>=0;i--){ const b=Bullets[i]; b.update(); const remove = bulletCollisionLogic(b); if(remove) Bullets.splice(i,1); }
...
<div class="row">
  ...
  <button id="adminGiveRandomCard">Give Random</button>
    ...
    <div class="row">
      <button id="adminTeleport">Teleport</button>
  ...
  document.getElementById('adminGiveCard').addEventListener('click', ()=>{
    ...
    if(card){ for(let i=0;i<amt;i++){ card.apply(p); p.cards.push(card.name); } alert('Gave '+amt+'x '+card.name); }
...
const PASS_URL = 'https://raw.githubusercontent.com/freesubstotally-beep/Password/refs/heads/main/password.html';
  ...
  const VERIFIED_KEY = '__admin_ultra_verified'; 
